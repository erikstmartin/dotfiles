...
_install_mise_tools() {
    ...
    # Pass 1: install runtimes first so their package managers (npm, gem, pipx, dotnet) are available
    mise install node ruby python dotnet rust go bun

    # Pass 2: install everything else (npm:, gem:, pipx:, dotnet:, cargo: tools, etc.)
    mise install
